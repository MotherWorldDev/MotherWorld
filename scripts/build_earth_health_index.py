...
def main():
    ...
    specs=[s for s in cfg.get('families',[]) if s.get('enabled',True)]; start=int(cfg.get('historyStartYear',1993)); mincov=float(cfg.get('minimumFamilyCoverage',.45)); thresholds=cfg.get('statusThresholds')
    ...
    if cfg.get('requireAllFamiliesForHistory', True) and start not in common:
    ...
    # Enforce a truly continuous annual slider; stop before the first gap.
    continuous=[]
    if common:
        y=max(start,common[0]); end=max(common)
        while y<=end and y in common: continuous.append(y); y+=1
    common=continuous
