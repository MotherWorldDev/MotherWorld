...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build Cesium geographic tiles from Black Marble 8-quadrant 21600x21600 TIFF set."
    )
    parser.add_argument("--source-dir", default="blackmarble")
    parser.add_argument("--prefix", default="BlackMarble_2016_")
    parser.add_argument("--suffix", default="_geo.tif")
    parser.add_argument("--out-dir", default="frontend/public/assets/imagery/black-marble-ultra-tiles")
    parser.add_argument("--levels", default="6", help="Comma-separated Cesium geographic levels, e.g. '6,7'")
...
def main() -> None:
    ...
    if any(level < 1 for level in levels):
        raise ValueError("Levels must be >= 1 for 4x2 quadrant layout")
        ...
        for code, (qx, qy) in QUADRANT_LAYOUT.items():
            ...
            for local_x in range(q_tiles_x):
                ...
                for local_y in range(q_tiles_y):
                    ...
                    save_image(
                        tile,
                        col_dir / f"{global_y}.{args.tile_format}",
                        args.tile_format,
